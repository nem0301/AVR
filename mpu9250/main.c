...
void spiWrite(unsigned char ss,
              unsigned char addr,
              unsigned char value)
{
    ...
    txBuffer[0] = addr;
    txBuffer[1] = value;
    spiTranceive(ss, rxBuffer, txBuffer, sizeof(txBuffer));
}

void spiRead(unsigned char ss,
             unsigned char addr,
             unsigned char *value)
{
    unsigned char txBuffer[2];
    unsigned char rxBuffer[2];
    txBuffer[0] = 0x80 | addr;
    txBuffer[1] = 0x00;
    spiTranceive(ss, rxBuffer, txBuffer, sizeof(txBuffer));
    *value = rxBuffer[1];
}
...
void func(char *args)
{
    ...
    else if (strcmp(cmd, "acc") == 0)
    {
        ...
        for (i = 0; i < 6; i++)
        {
            spiRead((unsigned char)atoi(argv[1]), 0x3b + i, &acc[i]);
        }
    ...
    {
        unsigned char ss = (unsigned char)atoi(argv[1]);
        unsigned char addr = (unsigned char)atoi(argv[2]);
        unsigned char data;
        spiRead(ss, addr, &data);
        printf("data %02x\n", data);
    }
    else if (strcmp(cmd, "wb") == 0)
    {
        unsigned char ss = (unsigned char)atoi(argv[1]);
        unsigned char addr = (unsigned char)atoi(argv[2]);
        unsigned char data = (unsigned char)atoi(argv[3]);
        spiWrite(ss, addr, data);
        spiRead(ss, addr, &data);
        printf("data %02x\n", data);
    }
...
int main(void)
{
    ...
    int addr = 9;
    eeprom_write_byte((uint8_t*)addr, 0x77);
    // main loop
    for(;;){
